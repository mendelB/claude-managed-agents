...
async function execute(ctx: Ctx, ev: ToolUseEvent): Promise<void> {
  ...
  try {
    if (!tool) {
      if (!isStock) {
        // Not ours — skip, do NOT post a result. An external orchestrator
        // this dispatcher has never heard of (e.g. MendelBot's L3 reviewer
        // fulfilling its `ctx_*` context tools out-of-band over the session
        // event stream) may own this custom_tool_use. Posting "not found"
        // for a tool we don't own races that fulfiller and poisons the
        // model with a spurious error alongside the real answer — see
        // notes/2026-06-08-l3-custom-tool-triple-fulfillment-bug.md. Stock
        // tools don't have this ambiguity: this dispatcher is now the sole
        // owner of the `agent.tool_use` namespace (no more `ant beta:worker
        // run` to race), so an unregistered stock name is a genuine
        // hallucination and falls through to the "not found" error below.
        const known = [...lookup.keys()].sort().join(",") || "(none)";
        console.log(
          `[microvm][custom-dispatch] kind=${kind} tool=${ev.name} not ours — skipping (registered=${known}) session=${ctx.sessionId} use_id=${ev.id}`,
        );
        return;
      }
      // Hallucinated/stale stock-tool name. Match the SDK runner's "not
      // found" wording so both surfaces agree to the model.
      const known = [...lookup.keys()].sort().join(",") || "(none)";
      console.warn(
